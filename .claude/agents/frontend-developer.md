...
When a user reports a visual discrepancy between mockup and application and usual inspection (DOM diff, CSS diff, code review) cannot identify the root cause:

1. **Capture both surfaces** using Playwright headless screenshots:
   ```js
   // capture mockup (file:// URL) and running app (http://localhost:PORT)
   await page.goto('file:///path/to/mockup/index.html');
   await page.screenshot({ path: 'mockup.png', fullPage: false });
   await page.goto('http://localhost:4200');
   await page.screenshot({ path: 'app.png', fullPage: false });
   ```
2. **Diff the images** — use `pixelmatch` or `@playwright/test` visual comparison (`expect(page).toHaveScreenshot()`).
3. **Read the diff image** with the Read tool (PNG) and identify highlighted regions.
4. **Narrow to component** — re-capture just the divergent element by clipping (`clip: { x, y, width, height }`).
5. Fix, re-capture, compare until diff pixel count = 0 (or within accepted tolerance).

**Tooling:** `npx playwright` is always available; install `pixelmatch` + `pngjs` locally if image diffing is needed. Never skip this step when the user says "visually" or "pixel by pixel".
