...
>
> **Code-intelligence routing.** Explore code via serena (symbol-level retrieval) and code-review-graph per the routing in `.claude/mcp-routing.md` (`CLAUDE.md` § *Code & docs intelligence (MCP)*); reach for `Read`/`Grep` on declarative files (json/yaml/csproj) or exact line ranges.
