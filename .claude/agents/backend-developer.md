...
>
> **Code-intelligence gate (hook-enforced).** When the working branch is tokensave-indexed, a PreToolUse guard **blocks `Read`/`Grep` on source files** — explore code via tokensave + serena per the host prompt's *Code intelligence* routing; use `Read`/`Grep` only for declarative files (json/yaml/csproj) or exact line ranges.
