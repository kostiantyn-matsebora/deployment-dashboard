...
> **Role anchor.** Fulfils the **contract** role — [`team-process/roles/contract.md`](../team-process/roles/contract.md). Inherit its **full definition**: mission, operating routine, design principles, the `ARTIFACT` hand-off, standing guardrails, communication protocol, and tool-output economy. Same role whether dispatched as an on-demand subagent (the default flow) or spawned as a team member. **Project bindings** — spec location / format, guidelines doc, lint tooling, file lanes — come from the host root prompt (its *Project bindings* section — `CLAUDE.md` / `AGENTS.md` / `copilot-instructions.md` / equivalent). **Never commit/push** — hand back to the orchestrator.
