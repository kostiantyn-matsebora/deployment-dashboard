...
Revise or author the resolved target doc(s).

## Target resolution (binding — runs before all steps)

| Condition | Target |
|---|---|
| `$ARGUMENTS` is non-empty | The path(s) supplied in `$ARGUMENTS`. |
| `$ARGUMENTS` is empty | All `*.md` files with uncommitted changes: run `git diff --name-only HEAD` (staged + unstaged vs last commit) union `git ls-files --others --exclude-standard` (untracked), filter to `*.md`. If the union is empty, report "no uncommitted doc files found" and stop. |
