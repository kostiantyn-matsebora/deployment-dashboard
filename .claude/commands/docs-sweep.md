...
Read-only by default. The **non-overwrite policy** from `.claude/agents/docs-keeper.md` still applies to any follow-up action you queue.

## README.md classification (binding)

Apply the binding classification from `.claude/agents/docs-keeper.md` § "README.md classification" before deciding what to flag. Default to content-bearing when ambiguous.
...
4. Flag any **un-listed content-bearing README.md**: a `README.md` classified as content-bearing (per `.claude/agents/docs-keeper.md` § "README.md classification") that is NOT in its nearest enclosing `index.md`'s `children:` (as `/README` for a sibling, or `/<sub>/README` for a deeper README inside a no-index sub-dir). Owner adds the entry — or `/docs-index <enclosing-dir>` will pick it up on next refresh.
...
7. Flag any **legacy navigation README.md** ONLY (per `.claude/agents/docs-keeper.md` § "README.md classification"): a `README.md` with only Files+TOC tables and no narrative/metadata, with a sibling `index.md`. Content-bearing READMEs are NEVER flagged here.
