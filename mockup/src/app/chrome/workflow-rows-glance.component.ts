// Workflow-rows × Glance — copy of workflow-rows-detailed with DeploymentGlanceComponent + 150×24 pills.

import { ChangeDetectionStrategy, Component, Input, signal } from '@angular/core';
import { CommonModule } from '@angular/common';
import { NgxGraphModule } from '@swimlane/ngx-graph';
import { DeploymentGlanceComponent } from './deployment-glance.component';
import {
...
@Component({
  selector: 'dd-mockup-workflow-rows-glance',
  standalone: true,
  imports: [CommonModule, NgxGraphModule, DeploymentGlanceComponent],
  ...
  `],
  template: `
    <main class="px-6 pt-4 pb-8" data-testid="pipeline-matrix" data-view="glance" data-layout="workflow-rows">
                ...
                @for (path of visiblePaths; track $index) {
                  <div class="ngx-graph-container" [attr.data-testid]="'workflow-row-' + sp.service.id + '-' + $index">
                    <ngx-graph
                      ...
                      [links]="path.links"
                      [layout]="'dagre'"
                      [layoutSettings]="{ orientation: 'LR', nodePadding: 6, rankPadding: 30, edgePadding: 12, ranker: 'tight-tree' }"
                      [nodeWidth]="150"
                      [nodeHeight]="24"
                      [animate]="false"
                      [autoZoom]="false"
                    ...
                    >
                      <ng-template #defsTemplate>
                        <svg:marker [attr.id]="'wrg-arrow-' + sp.service.id + '-' + $index" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto">
                          <svg:path d="M0,0 L10,5 L0,10 Z" class="arrow-head" />
                        </svg:marker>
                      </ng-template>
                      <ng-template #nodeTemplate let-node>
                        <svg:g class="node" ddDepGlance
                      ...
                      <ng-template #linkTemplate let-link>
                        <svg:g class="edge">
                          <svg:path class="line" stroke-width="1.2" [attr.marker-end]="'url(#wrg-arrow-' + sp.service.id + '-' + $index + ')'" [class.edge-correlated]="link.data?.source === 'correlated'" />
                        </svg:g>
                      </ng-template>
                    </ngx-graph>
                  </div>
...
export class WorkflowRowsGlanceComponent {
  ...
  toggleExpandAll(): void {
    if (this.allExpanded()) this.expanded.set(new Set());
    else this.expanded.set(new Set(this.servicesWithDeployments.map(s => s.id)));
  }
    ...
    return this.servicesWithDeployments.map(svc => {
      ...
      const allPaths: PathGraph[] = enumeration.paths.map(pathIds => {
        const nodes = pathIds.map(id => nodeById.get(id)!).filter(Boolean).map(n => ({
          ...n, dimension: { width: 150, height: 24 }
        }));
