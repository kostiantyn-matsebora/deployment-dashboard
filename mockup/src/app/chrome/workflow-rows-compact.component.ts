// Workflow-rows × Compact — copy of workflow-rows-detailed with DeploymentCompactComponent + 140×90.
...
@Component({
  selector: 'dd-mockup-workflow-rows-compact',
  standalone: true,
  imports: [CommonModule, NgxGraphModule, DeploymentCompactComponent],
  ...
  `],
  template: `
    <main class="px-6 pt-4 pb-8" data-testid="pipeline-matrix" data-view="compact" data-layout="workflow-rows">
                  ...
                  <div class="ngx-graph-container" [attr.data-testid]="'workflow-row-' + sp.service.id + '-' + $index">
                    ...
                    >
                      <ng-template #defsTemplate>
                        <svg:marker [attr.id]="'wrc-arrow-' + sp.service.id + '-' + $index" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto">
                      ...
                      <ng-template #linkTemplate let-link>
                        <svg:g class="edge">
                          <svg:path class="line" stroke-width="1.5" [attr.marker-end]="'url(#wrc-arrow-' + sp.service.id + '-' + $index + ')'" [class.edge-correlated]="link.data?.source === 'correlated'" />
                        </svg:g>
                      </ng-template>
                    </ngx-graph>
                  </div>
...
export class WorkflowRowsCompactComponent {
    ...
    return this.servicesWithDeployments.map(svc => {
      ...
      const allPaths: PathGraph[] = enumeration.paths.map(pathIds => {
        const nodes = pathIds.map(id => nodeById.get(id)!).filter(Boolean).map(n => ({
          ...n, dimension: { width: 140, height: 90 }
        }));
