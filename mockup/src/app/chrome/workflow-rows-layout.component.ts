...
// Flatten default path to ordered env-id array (for glance pill strip in workflow-rows).
function glanceEnvsForService(
  service: ServiceDescriptor,
  paths: readonly (readonly string[])[]
): readonly string[] {
  return paths[defaultPathIndex(paths, service, {})] ?? paths[0] ?? [];
}
...
@Component({
  ...
  template: `
    ...
    @if (viewMode === 'glance') {
      ...
      <!-- ════════════════════════════════════════════════════════════════════
           DETAILED / COMPACT / FOCUS VIEW — workflow rows with collapsed-default.
