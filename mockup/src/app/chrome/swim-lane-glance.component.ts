// Swim-lane × Glance — copy of swim-lane-detailed with DeploymentGlanceComponent + 150×24 pill nodes.
...
@Component({
  selector: 'dd-mockup-swim-lane-glance',
  standalone: true,
  ...
  `],
  template: `
    <main class="px-6 pt-4 pb-8" data-testid="pipeline-matrix" data-view="glance" data-layout="swim-lane">
      <div class="space-y-2">
                ...
                <div class="ngx-graph-container">
                  ...
                  >
                    <ng-template #defsTemplate>
                      <svg:marker id="glance-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto">
                    ...
                    <ng-template #linkTemplate let-link>
                      <svg:g class="edge">
                        <svg:path class="line" stroke-width="1.2" marker-end="url(#glance-arrow)" [class.edge-correlated]="link.data?.source === 'correlated'" />
                      </svg:g>
                    </ng-template>
                  </ngx-graph>
                </div>
...
export class SwimLaneGlanceComponent {
  ...
  graphFor(svc: ServiceWithDeployments): ServiceGraph {
    ...
    return {
      nodes: dag.nodes.map(n => ({ ...n, dimension: { width: 150, height: 24 } })),
      links: dag.edges
    };
  }
  ...
  viewSize(svc: ServiceWithDeployments): [number, number] {
    ...
    const width  = (maxRankIndex + 1) * 150 + maxRankIndex * 150 + 80;
    // Sum of node heights in tallest column + sum of gaps + small bottom margin.
    // Gap = nodePadding (6) from layoutSettings.
    const height = maxPerRank * 24 + (maxPerRank - 1) * 6 + 30;
    return [width, height];
  }
