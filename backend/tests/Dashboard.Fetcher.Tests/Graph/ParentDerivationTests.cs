...
// ── DeriveParents ─────────────────────────────────────────────────────────────────
public sealed class DeriveParentsTests
{
    // ── helpers ───────────────────────────────────────────────────────────────

    private static GhDeployment MakeDeployment(long id, string env) =>
        new() { Id = id, Environment = env, Ref = "main", Sha = "abc", Creator = null, CreatedAt = DateTimeOffset.UtcNow };

    private static WorkflowGraph MakeGraph(
        Dictionary<string, WorkflowJob> allJobs,
        Dictionary<string, WorkflowJob> deployJobs,
        string workflowName = "deploy")
        => new(workflowName, allJobs, deployJobs);

    private static Dictionary<long, Dictionary<string, string>> EnvMap(
        long runId, params (string Env, string GhDeployId)[] entries)
        => new()
        {
            [runId] = entries.ToDictionary(e => e.Env, e => e.GhDeployId)
        };

    // ── tests ─────────────────────────────────────────────────────────────────

    [Fact]
    public void DeriveParents_NullRunId_ReturnsEmpty()
    {
        var deployment = MakeDeployment(100, "prod");
        var allJobs = new Dictionary<string, WorkflowJob>
        {
            ["deploy-prod"] = new("deploy-prod", "prod", [])
        };
        var graph = MakeGraph(allJobs, allJobs);

        var result = ParentDerivation.DeriveParents(deployment, runId: null, graph, new());

        Assert.Empty(result);
    }

    [Fact]
    public void DeriveParents_NullGraph_ReturnsEmpty()
    {
        var deployment = MakeDeployment(100, "prod");

        var result = ParentDerivation.DeriveParents(deployment, runId: 42, graph: null, new());

        Assert.Empty(result);
    }

    [Fact]
    public void DeriveParents_NoMatchingDeployJob_ReturnsEmpty()
    {
        // Graph has no job targeting "prod" — nothing to derive.
        var deployment = MakeDeployment(100, "prod");
        var allJobs = new Dictionary<string, WorkflowJob>
        {
            ["deploy-dev"] = new("deploy-dev", "dev", [])
        };
        var graph = MakeGraph(allJobs, allJobs);

        var result = ParentDerivation.DeriveParents(deployment, runId: 1, graph, EnvMap(1));

        Assert.Empty(result);
    }

    [Fact]
    public void DeriveParents_LinearChain_ReturnsDirectParentId()
    {
        // dev → staging → prod: prod's direct deployment parent is staging.
        var deployment = MakeDeployment(300, "prod");

        var devJob = new WorkflowJob("deploy-dev", "dev", []);
        var stagingJob = new WorkflowJob("deploy-staging", "staging", ["deploy-dev"]);
        var prodJob = new WorkflowJob("deploy-prod", "prod", ["deploy-staging"]);

        var allJobs = new Dictionary<string, WorkflowJob>
        {
            ["deploy-dev"] = devJob,
            ["deploy-staging"] = stagingJob,
            ["deploy-prod"] = prodJob,
        };
        var deployJobs = allJobs;
        var graph = MakeGraph(allJobs, deployJobs);

        var envMap = EnvMap(
            runId: 5,
            ("dev", "gh-deploy-100"),
            ("staging", "gh-deploy-200"),
            ("prod", "gh-deploy-300"));

        var result = ParentDerivation.DeriveParents(deployment, runId: 5, graph, envMap);

        Assert.Single(result);
        Assert.Equal("gh-deploy-200", result[0]);
    }

    [Fact]
    public void DeriveParents_RunIdNotInEnvMap_ReturnsEmpty()
    {
        var deployment = MakeDeployment(100, "prod");
        var devJob = new WorkflowJob("deploy-dev", "dev", []);
        var prodJob = new WorkflowJob("deploy-prod", "prod", ["deploy-dev"]);
        var allJobs = new Dictionary<string, WorkflowJob>
        {
            ["deploy-dev"] = devJob,
            ["deploy-prod"] = prodJob,
        ...
        // envMap doesn't contain runId 99.
        var result = ParentDerivation.DeriveParents(deployment, runId: 99, graph, EnvMap(1));
