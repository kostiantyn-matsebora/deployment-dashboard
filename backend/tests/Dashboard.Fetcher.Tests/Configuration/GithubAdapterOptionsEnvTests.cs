...
/// <summary>
/// Unit tests for <see cref="GithubAdapterOptions.SplitRepo"/>.
/// </summary>
public sealed class GithubAdapterOptionsSplitRepoTests
{
    [Fact]
    public void SplitRepo_ValidOwnerSlashRepo_ReturnsBothParts()
    {
        var (owner, repo) = GithubAdapterOptions.SplitRepo("my-org/my-repo");
    ...
    [Fact]
    public void SplitRepo_NoSlash_ReturnsEmptyOwnerAndFullStringAsRepo()
    {
        var (owner, repo) = GithubAdapterOptions.SplitRepo("just-a-name");

        Assert.Equal("", owner);
        Assert.Equal("just-a-name", repo);
    }

    [Fact]
    public void SplitRepo_MultipleSlashes_SplitsOnFirstSlashOnly()
    {
        // "owner/repo/extra" should split at first slash: owner="owner", repo="repo/extra".
        var (owner, repo) = GithubAdapterOptions.SplitRepo("owner/repo/extra");

        Assert.Equal("owner", owner);
        Assert.Equal("repo/extra", repo);
    }

    [Fact]
    public void SplitRepo_LeadingSlash_ReturnsEmptyOwner()
    {
        var (owner, repo) = GithubAdapterOptions.SplitRepo("/repo-only");

        Assert.Equal("", owner);
        Assert.Equal("repo-only", repo);
    }
}
