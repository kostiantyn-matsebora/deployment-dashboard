...
- **Matrix layout removed from MVP; deferred to Phase 2.0.** Option C scope reduced to Swim-lane + Workflow-rows (the two MVP layouts). The canonical and `deployment-dashboard-version-C-hybrid.html` mockups no longer contain any `layout === 'matrix'` template blocks, matrix-only CSS (`.matrix-center`, `.arrow-col`, `.arrow-w-*`, `.arrow-line.arrow-line-dashed`), or the Matrix entry in the layout switcher / LAYOUTS const. The localStorage default flips from `'matrix'` to `'swim-lane'`.
- Phase 4 (canonical merge + SPA implementation) takes `deployment-dashboard-version-C-hybrid.html` as the merge source.
