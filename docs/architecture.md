...
| `POST` | `/api/fetcher/usage` | `200 / 422` | **Write group — pull-mode adapter only** (added by [CR-0011](cr/CR-0011-fetcher-rate-limit-governance.md)). Auth-gated by `X-Api-Key`. **Requires** request header `X-Progress-Reporter` (≤ 64, non-whitespace). Fetcher pushes its current upstream rate-limit observation + self-imposed cap on every poll tick. Body: `{ "adapter_id", "source_id", "upstream_limit", "upstream_remaining", "upstream_reset_at", "self_imposed_cap", "upstream_used", "observed_at" }`. `422` on missing / over-cap fields. Returns `200 OK` with empty body. Backend caches the latest snapshot per `(adapter_id, source_id)` in-memory; no persistence (re-publish-on-tick recovers the cache after replica restart). See paired [ADR-0008](adr/ADR-0008-leaky-bucket-cap-and-republish-on-tick.md). |
| `GET` | `/api/fetcher/usage` | `200 OK` | **Read group** (added by [CR-0011](cr/CR-0011-fetcher-rate-limit-governance.md)). No auth. Returns the latest cached snapshot per `(adapter_id, source_id)` as `{ "snapshots": [ { ...request fields..., "received_at": "<iso-8601 UTC>" } ] }`. Empty array `{ "snapshots": [] }` when no fetcher has pushed yet — never `404`. `received_at` is the server-side timestamp at POST landing; lets clients stale-out the cluster after `2 × poll_interval` without depending on the SSE wire (per CR-0003 SSE-carries-slot-updates-only boundary). |
...
| 12 | Graph + connector rendering library, layout mode, custom-node-template contract, edge-style mapping, disconnected-component handling | **`@swimlane/ngx-graph` + dagre (`LR`)** for all three layouts (Swim-lane / Workflow-rows / Glance — Glance is the degenerate single-row case). Custom-node-template wraps the existing `<dd-layout-leaf>`; fixed per-viewMode dimensions; edges styled per `(topology.edges[].source, target slot status)` with light + dark theme tokens; disconnected sub-components render as N stacked ngx-graph instances per service. See [ADR-0012](adr/ADR-0012-ngx-graph-layout-and-node-template-contract.md). |
