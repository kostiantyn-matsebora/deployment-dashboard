...
| G2 | **No fetcher code change** — fetcher is config-driven (FETCHER_SPEC F9); demo mode sets `GITHUB_BASE_URL=http://github-emulator:3100`. The fetcher-host is wired into the demo compose profile (currently absent from all compose files). | Config-only change; fetcher adapter is unmodified. |
...
> The emulated GitHub REST surface and its config (e.g. `GITHUB_SIM_RATE_LIMIT`) are owned by the emulator — see [`GITHUB_EMULATOR_SPECIFICATION.md`](GITHUB_EMULATOR_SPECIFICATION.md) §4. The fetcher's own demo-mode config (`GITHUB_BASE_URL`, `GITHUB_TOKEN`, etc.) is fetcher config — see FETCHER_SPEC §6.
