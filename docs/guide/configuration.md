...
|---|---|---|---|
| `DASHBOARD_VERSION` | no | `latest` | Image tag applied to all six stack images. **Set without a leading `v`** — the git tag `v0.11.0` publishes images as `0.11.0`. `latest` tracks the newest push to main. For a reproducible deploy, pin to a published release (e.g. `0.11.0`). |
