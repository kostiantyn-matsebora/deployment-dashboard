...
```bash
docker compose --project-directory . -f oci://ghcr.io/kostiantyn-matsebora/deployment-dashboard-compose-demo:0.13.1 --profile demo up
```
...
To pin to a specific release, replace `main` in the URLs with the release tag (e.g. `.../v0.13.1/compose/...`) and set `DASHBOARD_VERSION=0.13.1` for a reproducible deploy — see [Pinning a release version](./install.md#pinning-a-release-version).
