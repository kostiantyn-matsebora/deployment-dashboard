---
title: Diagrams
shortTitle: Diagrams
intro: 'Architecture and process diagrams — visual references; authoritative behaviour stays in the spec docs.'
children:
  - /architecture-c4
  - /fetcher-rate-limit
  - /github-emulation
  - /reset-choreography
---

## Contents — `architecture-c4.md`

- [Component diagram (C4)](./architecture-c4.md#component-diagram-c4)

## Contents — `fetcher-rate-limit.md`

- [Diagram A — Emit and fan-out sequence](./fetcher-rate-limit.md#diagram-a--emit-and-fan-out-sequence)
- [Diagram B — Consumer topology](./fetcher-rate-limit.md#diagram-b--consumer-topology)
- [Payload — `event_type: "rate-limit"`](./fetcher-rate-limit.md#payload--event_type-rate-limit)
- [Decisions (locked)](./fetcher-rate-limit.md#decisions-locked)

## Contents — `reset-choreography.md`

- [Control-stream event vocabulary](./reset-choreography.md#control-stream-event-vocabulary)
- [Sequence](./reset-choreography.md#sequence)
- [State machine (API-driven)](./reset-choreography.md#state-machine-api-driven)
- [Decisions (locked)](./reset-choreography.md#decisions-locked)

## Contents — `github-emulation.md`

- [Diagram A — Demo-mode topology](./github-emulation.md#diagram-a--demo-mode-topology)
- [Diagram B — Seed → backfill → poll sequence](./github-emulation.md#diagram-b--seed--backfill--poll-sequence)
- [Decisions (locked)](./github-emulation.md#decisions-locked)
