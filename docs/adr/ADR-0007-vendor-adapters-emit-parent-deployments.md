...
- **Context.**
  ...
  - **No wire-contract change.** `DeploymentEventRequest.ParentDeployments` already exists per CR-0003 + the canonical DTO doc. This decision changes how an adapter fills it — not what is on the wire. Push-path adopters' current behaviour (set the array explicitly themselves) is the precedent.
  ...
  - **NFR-02 (≤ $30/month) — HTTP-call budget.** Each adapter implementing this ADR will add cacheable, per-cycle endpoint calls (for GHA: `/actions/runs/{id}/jobs` + workflow YAML contents). The added calls must be documented in the adapter's owning CR (CR-0009 §3d for GHA) so the per-cycle endpoint budget remains auditable.
...
- **Consequences.**
  ...
  - **Per-adapter discipline.** For each new vendor adapter (today: GHA per CR-0009 §3d + issue [#19](https://github.com/kostiantyn-matsebora/deployment-dashboard/issues/19); tomorrow: ADO, Jenkins, GitLab, CircleCI), the implementing engineer must re-confirm the **negative-space rule** explicitly before writing the conversion: "what does this vendor *assert* vs. *render*?" Only the asserted lineage is converted. This goes in the adapter's owning CR or feature issue alongside the endpoint-list addition.

  - **Endpoint-budget growth, documented per adapter.** Each adapter implementing this ADR will add cacheable, per-cycle endpoint calls (for GHA: `/actions/runs/{id}/jobs` per distinct workflow-run-id seen in a cycle; `/repos/{o}/{r}/contents/.github/workflows/{file}` per distinct workflow-id × workflow-sha seen in a cycle). The added endpoints must be appended to the adapter's owning CR (CR-0009 §3d for GHA — amended in parallel with this ADR per § Cross-references). Future adapters (ADO / Jenkins / GitLab / CircleCI) must amend their own CR when filed. NFR-02 (≤ $30/month) remains in budget: GHA's authenticated rate limit is 5000/hr, and cacheable per-run + per-workflow calls stay well inside it.
...
- **References.**
  ...
  - [CR-0009](../cr/CR-0009-pull-mode-fetcher-and-progress-reporter.md) — Fetcher charter. §3d's endpoint list is amended in parallel with this ADR to add `/actions/runs/{id}/jobs` + workflow YAML contents endpoints (the GHA adapter's first instantiation of this ADR's posture). Future adapters' CRs must amend their own endpoint lists similarly.
  - [CR-0003](../cr/CR-0003-tree-topology-and-layout-axis.md) — Source of `deployment_id` + `parent_deployments` on the wire and the per-service topology + correlation-attribute precedence the read-side five-pass consumes. Unchanged.
