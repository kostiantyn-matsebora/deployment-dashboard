...
- **Consequences.**
  ...
  - **No `--profile migrate` UX trap.** Installers run `docker compose up -d --wait`; pending migrations apply unconditionally as the API host starts.
  - **No sixth release asset.** Release tarball shrinks by one file; release pipeline drops the dotnet-setup chain + the inline `dotnet ef migrations script` step; reusable-workflow `emit-migration-artefact` machinery deleted.
  ...
  - **No installer flag.** A bad migration that aborts startup is loud (process exit non-zero, ACA replica restarts in `CrashLoopBackOff`-equivalent) instead of silent (forgotten `--profile migrate` against an unmigrated DB). Faster diagnosis.
