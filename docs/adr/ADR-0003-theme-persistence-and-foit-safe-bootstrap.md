...
- **Status:** accepted
- **Context:** The Light / Dark / Auto theme requirement introduced a third orthogonal UX axis — Theme — with three values (`light` / `dark` / `auto`) and the default `auto`. Two architectural questions arise:
  ...
  - **Mockup ↔ SPA parity (single-writer invariant)** — after bootstrap, exactly one writer mutates `<html data-theme>` / `<html data-theme-pref>`. The mechanism must scale from the mockup (Alpine.js root) to the Angular SPA (`ThemeService` in `frontend/shared/`).
...
- **References:**
  - SAD §7 "Visual layout → Client-side persistence (`localStorage`)" — `dashboard.theme` key.
  - SAD §7 "Visual layout → Theme axis (presentation-only)" — single-writer invariant, corruption normalisation, mockup-vs-implementation parity.
