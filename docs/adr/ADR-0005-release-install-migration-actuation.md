...
- **Consequences.**
  ...
  - **Re-install idempotency is operationally guarded by `install.ps1`'s `dashboard.env`-aware secret read.** Per issue [#37](https://github.com/kostiantyn-matsebora/deployment-dashboard/issues/37), the installer (a) defaults `-InstallDir` to a CWD-independent `~/.dashboard-release`, (b) preserves `POSTGRES_PASSWORD` + `API_TOKEN` whenever a prior `dashboard.env` is discoverable, and (c) refuses to proceed with a fresh env-file when a pre-existing `pg-data` volume is detected without a matching `dashboard.env` (would silently rotate the cluster password). The `--idempotent` migration contract above is the migration-actuation side of the same upgrade story; this bullet captures the secret-handling side.
...
- **References.**
  ...
  - Component CI pipeline (historical CR-0010) — defines the four component images and the `_build-and-push-image.yml` reusable workflow this ADR extends with a sibling `release.yml`.
  ...
  - [ADR-0006](./ADR-0006-microservices-architecture-with-container-co-location.md) (supersedes [ADR-0002](./ADR-0002-modular-monolith-consolidation.md) on framing; ADR-0002 retains the co-location mechanics) — microservices architecture with the Write + Read API services co-located in `deployment-dashboard-api`. The "one API container today" packaging choice survives unchanged (this ADR adds no code, only an external migration-actuation mechanism).
