...
- **Amendments:**
  - **2026-05-20** — §3d IN-list expanded to add `GET /repos/{o}/{r}/actions/runs/{run_id}/jobs` and `GET /repos/{o}/{r}/contents/.github/workflows/{workflow_file}` so the GHA adapter can recover deployment-producing jobs and parse the workflow-run `needs:` DAG for `parent_deployments` edge emission. Posture and negative-space rule (what to convert / what to defer to the read-side five-pass / why timing inference is rejected) recorded in paired [ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md); implementation tracked under issue [#19](https://github.com/kostiantyn-matsebora/deployment-dashboard/issues/19). Surgical amendment — §3d IN-list only; status unchanged; no other CR-0009 surface affected.
...
- **Change.** Two co-introduced amendments and three framing decisions:
  ...
  - **3d — MVP scope.**
    - **IN:** GitHub Actions adapter only:
      ...
      - `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` — recover deployment-producing jobs in a workflow run, for intra-run `needs:` edge emission per [ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md) (added 2026-05-20; implementation tracked under issue [#19](https://github.com/kostiantyn-matsebora/deployment-dashboard/issues/19)).
      - `GET /repos/{owner}/{repo}/contents/.github/workflows/{workflow_file}` — recover workflow YAML for `needs:` declaration parsing, per the same ADR + issue.

      Plus: cursor endpoints (`GET`/`PUT /api/fetcher/state/{source-id}`); fetcher host (ASP.NET Core Worker on `Microsoft.NET.Sdk.Worker`); Dockerfile; opt-in `docker-compose.local.yml --profile fetcher` entry; ACR image build; `X-Progress-Reporter` on `POST /api/deployments` plus persistence to the new `progress_reporter` column plus surfacing on the existing event-attribute Read responses; first-fetch cap (`INITIAL_FETCH_LIMIT` default 50, ceiling 500). The two new endpoints are cacheable per fetch cycle (jobs API: per distinct `workflow_run_id`; workflow contents: per distinct `(workflow_id, workflow_sha)`) and silently degrade on failure (INFO log; no edges emitted from the affected signal; fetch cycle does not fail — per [ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md) § Decision rule 2).
