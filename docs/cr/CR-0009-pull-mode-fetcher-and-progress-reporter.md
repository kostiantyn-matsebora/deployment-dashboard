...
- **Amendments:**
  - **2026-05-20** — §3d IN-list expanded to add three GHA endpoints needed for intra-run `needs:` recovery: `GET /repos/{o}/{r}/actions/runs/{run_id}` (run metadata — workflow `path` + `head_sha`), `GET /repos/{o}/{r}/actions/runs/{run_id}/jobs` (deployment-producing job → name map), and `GET /repos/{o}/{r}/contents/.github/workflows/{path}?ref={head_sha}` (workflow YAML at the exact run revision). Together they let the GHA adapter recover the workflow-run `needs:` DAG for `parent_deployments` edge emission against the YAML *as the run saw it* (vs. as it is now on the default branch). Posture and negative-space rule (what to convert / what to defer to the read-side five-pass / why timing inference is rejected) recorded in paired [ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md); implementation tracked under issue [#19](https://github.com/kostiantyn-matsebora/deployment-dashboard/issues/19). Surgical amendment — §3d IN-list only; status unchanged; no other CR-0009 surface affected.
...
- **Change.** Two co-introduced amendments and three framing decisions:
  ...
  - **3d — MVP scope.**
    - **IN:** GitHub Actions adapter only:
      - `GET /repos/{owner}/{repo}/deployments` + `GET /repos/{owner}/{repo}/deployments/{id}/statuses` — base deployment + status retrieval (cursor = highest seen `deployment.id`).
      - `GET /repos/{owner}/{repo}/actions/runs/{run_id}` — recover workflow YAML `path` + `head_sha` for the run, so the YAML can be fetched at the exact commit revision the run executed against (a later edit on the default branch must not change the `needs:` attributed to a past run). Prerequisite to endpoints 4 + 5 below for intra-run `needs:` edge emission per [ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md) § Decision rule 1 (added 2026-05-20; implementation tracked under issue [#19](https://github.com/kostiantyn-matsebora/deployment-dashboard/issues/19)).
      - `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` — recover deployment-producing jobs in a workflow run (map `job_id` → `job_name`) for intra-run `needs:` edge emission per the same ADR + issue.
      - `GET /repos/{owner}/{repo}/contents/.github/workflows/{path}?ref={head_sha}` — recover workflow YAML *at the run revision* (parameters from the run-metadata response) for `needs:` declaration parsing, per the same ADR + issue.

      Plus: cursor endpoints (`GET`/`PUT /api/fetcher/state/{source-id}`); fetcher host (ASP.NET Core Worker on `Microsoft.NET.Sdk.Worker`); Dockerfile; opt-in `docker-compose.local.yml --profile fetcher` entry; ACR image build; `X-Progress-Reporter` on `POST /api/deployments` plus persistence to the new `progress_reporter` column plus surfacing on the existing event-attribute Read responses; first-fetch cap (`INITIAL_FETCH_LIMIT` default 50, ceiling 500). The three new endpoints are cacheable per fetch cycle (run metadata: per distinct `workflow_run_id`; jobs API: per distinct `workflow_run_id`; workflow contents: per distinct `(workflow_path, head_sha)`) and silently degrade independently on failure (INFO log; no edges emitted from the affected signal; fetch cycle does not fail — per [ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md) § Decision rule 2).
  ...
  - **3e — Naming.** Library project `Dashboard.Fetcher`; host project `Dashboard.Fetcher.Host`; container image `deployment-dashboard-fetcher`. Both projects live under a new `backend/fetcher/` and `backend/fetcher-host/` directory tree; Dockerfile under `backend/fetcher-host/`. **Image-name vs. reporter-value asymmetry (deliberate).** The recommended `X-Progress-Reporter` value (`dashboard-fetcher/<adapter>`, per § 3a) intentionally keeps the legacy `dashboard-*` prefix while the container image is renamed to `deployment-dashboard-fetcher`. The reporter value is a free-form wire-shape string visible in operator log search (renaming it would churn every existing log query); the image name is an artefact-namespace identifier in GHCR. The two namespaces are independent by design.
...
- **[ADR-0007](../adr/ADR-0007-vendor-adapters-emit-parent-deployments.md)** — vendor adapters convert vendor correlation signals into `parent_deployments` edges; read-side five-pass remains backstop. Amends this CR's §3d endpoint list verbatim (2026-05-20) for the GHA adapter's `needs:` recovery; future vendor adapters' CRs must amend their own endpoint lists similarly.
