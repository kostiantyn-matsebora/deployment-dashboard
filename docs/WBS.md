...
- 1.6 Component CI (per [CR-0010](cr/CR-0010-component-ci-pipeline.md), paired with [`docs/ci-cd-pipelines.md`](ci-cd-pipelines.md)). Outbound component-CI track — how the dashboard's own four container images are built, tested, and published. Inbound integration (third-party pipelines pushing to the dashboard) is the separate §1.4 track. Locked decisions: hybrid topology (4 thin callers + 1 reusable workflow); container images only (no NuGet / npm); GHCR today, ACR-cutover one-input swap when §4.3 lands; CI-only (no deploy step); identical trigger matrix across all four workflows (push:main / pr:main / tags v* / workflow_dispatch).
  ...
  - 1.6.3 `.config/dotnet-tools.json` — pins `dotnet-ef` to the .NET 10 release line for local-dev migration authoring (`dotnet ef migrations add` from the host). Retained post-[ADR-0009](adr/ADR-0009-startup-applied-ef-migrations.md) as a contributor-tooling pin; CI no longer invokes `dotnet ef migrations script` (the API host self-applies migrations on startup). **Owner:** `devops-engineer`.
  ...
  - 1.6.6 `backend/api/Dashboard.Api/Dashboard.Api.csproj` — `Microsoft.EntityFrameworkCore.Design` PackageReference with `PrivateAssets=all`. Retained post-[ADR-0009](adr/ADR-0009-startup-applied-ef-migrations.md) for local-dev migration authoring (`dotnet ef migrations add` resolves the design-time `DbContext` from this reference); CI no longer runs `dotnet ef migrations script` per ADR-0009 (the API host self-applies migrations on startup via `Database.Migrate()`). `PrivateAssets=all` keeps the design-time package off the runtime closure — tests / image / runtime unaffected. **Owner:** `backend-engineer`.
...
- 2.1 `docker-compose.yml` — API container (Write + Read surfaces) + Dashboard Frontend + App Gateway + PostgreSQL + pgAdmin. The API host self-applies pending EF migrations on startup per [ADR-0009](adr/ADR-0009-startup-applied-ef-migrations.md); no external `migrations` one-shot service.
...
- 5.3 Database migration actuation — API host self-applies pending EF migrations on startup per [ADR-0009](adr/ADR-0009-startup-applied-ef-migrations.md); no separate deployment-pipeline step. Verified by `backend/api/Dashboard.Api.Tests/StartupMigrationTests.cs` (startup-applied path; idempotent re-apply; failure aborts the process before `app.Run()`).
...
- 8.2 Database schema migration runs implicitly on first API replica startup per [ADR-0009](adr/ADR-0009-startup-applied-ef-migrations.md); no separate deploy step. Failure mode: a bad migration aborts the API process before `app.Run()`, ACA restarts the replica in `CrashLoopBackOff`-equivalent — operator inspects ACA replica logs for the `Critical`-level EF error and rolls back to the previous image tag (which carries the previous migration set). No `migration.sql` artefact is materialised on the host; recovery is image-tag-pin only.
