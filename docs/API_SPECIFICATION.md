...
| `payload` | jsonb | yes | opaque; mirrors `component_events.payload`. `null` on reset frames; carries the resolved rewind point on every `recover-*` frame (`{"since":"…"}`, D18) |
