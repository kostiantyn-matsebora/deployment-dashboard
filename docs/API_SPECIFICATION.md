...
| meta | `GET /api/version` | none | deployed build version → `{ version }`; value is stamped into the API assembly at image-build time, or `0.0.0-dev` fallback for unstamped (local/dev, `:latest`/`main`) builds; for the SPA footer |
