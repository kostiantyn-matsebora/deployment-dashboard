...
| `mockup/` (standalone Angular 20 mockup-app — own `package.json` + own `node_modules` + own build; port 4201; hand-authored Tailwind chrome + hardcoded fixtures; ZERO `@dd/*` consumption per ADR-0011 amendment) | Standalone visual contract + PoC sandbox per CR-0015 + ADR-0011 | `frontend-engineer`; `solution-architect` reviews for architectural coherence |
...
|   |-- matrix/          pipeline matrix + view templates + attribute picker + (post issue #79) migrated header + swim-lane-layout + workflow-rows-layout + topology-utils
|   |-- drawer/          history drawer
|   `-- shared/          Signal Store + API client + SSE service + models + fixtures
|-- mockup/              standalone Angular 20 mockup-app per CR-0015 / ADR-0011 - zero @dd/* deps, hand-authored Tailwind chrome, hardcoded fixtures, port 4201; 7 routes (canonical + variant + /invariants)
...
| Mockup-app (standalone tier) | Angular 20 standalone + Tailwind CSS + Karma at `mockup/` (port 4201; zero `@dd/*` deps; own `package.json` + `node_modules` per CR-0015 / ADR-0011 amendment) |
...
| `frontend-engineer` (alias `client-engineer`) | `frontend/` (Angular workspace - dashboard / matrix / drawer / shared); `mockup/` (standalone Angular 20 mockup-app per CR-0015 / ADR-0011 - hand-authored Tailwind chrome + hardcoded fixtures + variant routes); `docs/ui/mockups/*.html` (legacy HTML mockup + variants - retire under sibling issue #80); Signal Store; Tailwind styling; client-side fetch / SSE; per-option mockup proposals under `docs/ui/`; per-app docs (D25 per-tier reassignment). |
