...
async function loadFilterLists(
  ...
  try {
    ...
    const services: string[] = svcRes.ok ? await svcRes.json() : [];
    const environments: string[] = envRes.ok ? await envRes.json() : [];
...
inputUrl.addEventListener('change', async () => {
  ...
  if (url) {
    const currentServices = collectChecked('svc');
    const currentEnvs = collectChecked('env');
    await loadFilterLists(url, currentServices, currentEnvs);
  }
});
